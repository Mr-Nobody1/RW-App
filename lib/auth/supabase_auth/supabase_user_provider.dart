...
class ReflectionWomenSupabaseUser extends BaseAuthUser {
  ReflectionWomenSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> reflectionWomenSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ReflectionWomenSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
